...
def test_testcase1_successful_login_logout(driver):
    test_valid_login_credentials(driver)
    user_logout = dashboard(driver)
    user_logout.logout()


#TC-04 validate visibility and click ability of main menu items after login
...
def test_verify_new_user_in_table(driver):
    test_valid_login_credentials(driver)
    # Add new user
    new_user_creation = UserManagement(driver)
    new_user_creation.addnewuser()
    #verity newly created user is displayed in the table
    dashboard_page = dashboard(driver)
    dashboard_page.is_username_present_in_table()
...
#TC-09 Assign leave to employee
...
def test_claim(driver):
    ...
    test_new_user_creation(driver)
    #intiate claim and verify the same is getting displayed in the table
    claim_creation = ClaimPage(driver)
